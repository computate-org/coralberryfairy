
CREATE TABLE IF NOT EXISTS SiteUser();
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS pk bigserial primary key;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS created timestamp with time zone;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS archived boolean;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS sessionId text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userKey bigint;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS objectTitle text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS displayPage text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userId text UNIQUE;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userName text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userEmail text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userFirstName text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userLastName text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS userFullName text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS seeArchived boolean;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS awesomeEffect boolean;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS displayName text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS siteFontSize text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS siteTheme text;
ALTER TABLE SiteUser ADD COLUMN IF NOT EXISTS webComponentsTheme text;

CREATE TABLE IF NOT EXISTS Doll();
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS pk bigserial primary key;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS created timestamp with time zone;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS archived boolean;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS sessionId text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS userKey bigint;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS objectTitle text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS displayPage text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS name text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS description text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS price decimal;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS pageId text UNIQUE;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS resourceUri text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS templateUri text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS emailTemplate text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS storeUrl text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS instagramUrl text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS title text;
ALTER TABLE Doll ADD COLUMN IF NOT EXISTS productNum integer;
